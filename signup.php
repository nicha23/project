...
<body>
	<form action="signup1.php" method="post">
		...
		<div class="container">
			...
			<input type="text" placeholder="Enter Account No." name="AccountNo" style="width: 48.5%" required>

			<label for="acctype"><b> Account Type</b></label>
			<select name="acctype" style="width: 30%" required>
				<option value="" selected="selected" disabled="disabled">-- select one --</option>
				<?php
				$sql = "SELECT TypeAccountID,TypeAccount FROM typeaccount";
				if ($result = mysqli_query($conn, $sql)) {
					while ($row = mysqli_fetch_array($result)) {
			                    // printf ("%s (%s)\n", $row['TypeAccountID'], $row['TypeAccount']);
						?>
						<option value="<?php echo $row['TypeAccountID']?>"><?php echo $row['TypeAccount']?>
							<?php
						}
						$result->close();
					}
				...
				<select name="branch" required>
					<option value="" selected="selected" disabled="disabled">-- select one --</option>
					<?php
					$sql = "SELECT BranchID,BranchName FROM branchinfo";
					if ($result = mysqli_query($conn, $sql)) {
						while ($row = mysqli_fetch_array($result)) {
			                    // printf ("%s (%s)\n", $row['TypeAccountID'], $row['TypeAccount']);
							?>
